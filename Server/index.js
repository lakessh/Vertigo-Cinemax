  ...
  db.getByQuery(searchTerm, function(data) {
    if (data != null){  // Array.isArray(data) && data.length) {  //data is array && not []
      ...
      res.end();
        ...
        movies.search(searchTerm, function(moviedata) {
            wrapping(res, moviedata.code, moviedata, req.headers.origin)
            moviedata.query = searchTerm;     //add searchTerm to moviedata for addQuery to save searchTerm
            db.addQuery(moviedata, dbResult => {
...
server.get('/movies/find/:id', function(req, res) {
  ...
  db.getByMovieId(movieid, function(data) {
    if (data != null){  // Array.isArray(data) && data.length) {  //data is array && not []
      ...
      movies.searchMovieInfo(movieid, function(moviedata) {
         wrapping(res, moviedata.code, moviedata, req.headers.origin)

          moviedata.query = movieid;     //add searchTerm to moviedata for addQuery to save searchTerm
          db.addMovie(moviedata.data, dbResult => {
              console.log('MongoDB: '+ dbResult);
          })
      })
    }
